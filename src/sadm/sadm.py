...
def train():
    ...
    dataset = StarmenDataset(data_dir=DATA_DIR, split="train", nb_subject=10)
    train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=1)

    valid_loader = DataLoader(StarmenDataset(data_dir=DATA_DIR, split="val", nb_subject=2), batch_size=batch_size, shuffle=False, num_workers=1)
    x_val, x_prev_val = next(iter(valid_loader))
    x_prev_val = x_prev_val.to(device)


    vivit_model = ViViT(image_size, patch_size, num_frames)
    nn_model = ContextUnet(in_channels=1, n_feat=n_feat, in_shape=(1, *image_size), is_3d=is_3d)
