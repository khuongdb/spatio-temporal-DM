...
import torch
from src.ldae.modules.transformers import TransformerEncoderLayer, TransformerEncoder
...
class PositionalEncoding(torch.nn.Module):
    ...
    def forward(self, x, rel_times):
        if rel_times is None:
            ...
            position = rel_times.unsqueeze(2)  # shape: (batch, seq_len, 1)
...
class LongTransformer(torch.nn.Module):
    def __init__(self, encoder, args, device):
        super(LongTransformer, self).__init__()
        self.device = device
        self.args = args
        ...
        transformer_encoder = TransformerEncoderLayer(d_model=self.encoder.embedding_dim, nhead=args.n_heads, dim_feedforward=self.encoder.embedding_dim, dropout=args.dropout, activation='relu', batch_first=True)
        self.transformer = TransformerEncoder(transformer_encoder, num_layers=args.n_layers)
        # else:
        #     transformer_encoder = torch.nn.TransformerEncoderLayer(d_model=self.encoder.embedding_dim, nhead=args.n_heads, dim_feedforward=self.encoder.embedding_dim, dropout=args.dropout, activation='relu', batch_first=True)
        #     self.transformer = torch.nn.TransformerEncoder(transformer_encoder, num_layers=args.n_layers)

        if args.learned_pe:
            self.pos_encoder = LearnedPositionalEncoding(d_model=self.encoder.embedding_dim)
        else:
            if args.tpe: 
                self.pos_encoder = PositionalEncoding(d_model=self.encoder.embedding_dim, dropout=0, max_len=args.max_seq_len) 
            else:
                self.pos_encoder = PositionalEncoding(d_model=self.encoder.embedding_dim, dropout=0, max_len=args.max_seq_len)

        # # Use for survival analysis - NOT USE IN cDDPM model
        # self.classifier = torch.nn.Sequential(
        #     torch.nn.Dropout(p=args.dropout),
        #     torch.nn.Linear(self.encoder.embedding_dim, args.n_classes),
        #     torch.nn.Sigmoid()
        # )

        if args.step_ahead:
            self.rel_pos_encoder = PositionalEncoding(d_model=self.encoder.embedding_dim, dropout=0, max_len=args.max_seq_len)

            self.step_ahead_predictor = torch.nn.Sequential(
                torch.nn.Dropout(p=args.dropout),
                torch.nn.Linear(self.encoder.embedding_dim, self.encoder.embedding_dim)
            )

        self.causal_mask = torch.triu(torch.full((args.max_seq_len, args.max_seq_len), float('-inf'), device=self.device), diagonal=1)

    def forward(self, x, seq_lengths, rel_times):
        ...
        # # Survival analysis - Does not use in our model
        ...
        if self.args.step_ahead:
            ...
            if self.args.attn_map:
                return feat_preds, feat_targets, padding_mask, attn_map
            else:
                return feat_preds, feat_targets, padding_mask

        if self.args.attn_map:
            return padding_mask, attn_map
        else:
            return padding_mask
