...
from src.ldae.base_nets import LitRegression3DBase
import torch.nn as nn
import torch


class EncoderRegressor(LitRegression3DBase):
    """
    This module is used to perform regression from 3D images. It uses a backbone to extract features
    from the images, then it computes a global representation of the input using soft attention. The global vector is
    then projected into Q, K, V and used to compute self-attention with cross-attention. Finally, it applies a linear
    regressor to the output of the attention fusion.
    !!!Note: See the LitRegression3DBase class for the inherited attributes and methods.
    Args:
        ...
        pretrained_semantic_path (str): Path to the pretrained semantic encoder weights.
    Inputs:
        x (torch.Tensor): The input of the module with shape `(batch_size, num_slices, 3, 224, 224)`.
    Outputs:
        logits (torch.Tensor): The output of the module with shape `(batch_size, 1)`.
    """
    ...
    def __init__(self,
        ...
        # Classifier
        self.classifier = nn.Linear(self.embedding_dim, 1)  # Final linear layer
