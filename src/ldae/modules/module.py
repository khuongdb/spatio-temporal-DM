...
class TimestepContextBlock(nn.Module):
    ...
    def forward(self, x, emb, context):
        """
        Apply the module to `x` given `emb` timestep embeddings and `context` conditions.
        """


class TimestepSequential(nn.Sequential, TimestepBlock, TimestepContextBlock):
    def forward(self, x, emb, context=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, TimestepContextBlock):
