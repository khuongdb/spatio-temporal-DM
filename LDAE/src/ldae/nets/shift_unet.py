...
class ShiftUNet(nn.Module):
    ...
    def set_refining_mode(self):
        self.time_embed.train()
        self.input_blocks.train()
        self.middle_block.train()
        self.output_blocks.train()
        self.out.train()

        self.time_embed.requires_grad_(requires_grad=True)
        self.input_blocks.requires_grad_(requires_grad=True)
        self.middle_block.requires_grad_(requires_grad=True)
        self.output_blocks.requires_grad_(requires_grad=True)
        self.out.requires_grad_(requires_grad=True)

        self.set_eval_mode()

        self.label_emb.requires_grad_(requires_grad=False)
        self.shift_middle_block.requires_grad_(requires_grad=False)
        self.shift_output_blocks.requires_grad_(requires_grad=False)
        self.shift_out.requires_grad_(requires_grad=False)
